...
2. 阶段 1.5：先做错误类型诊断，再决定是否进入增强。必须区分候选召回失败、候选命中但 LLM 误判、输出解析失败、API 调用失败、token 预算失败。
3. 第二阶段调整为“候选召回与 prompt 参数小步实验”，而不是完整 prompt compiler。当前本地 DEV 诊断显示候选集 gold 命中率 94.2%，top20 召回 96.5%，平均候选数 13.6，说明检索仍有改进空间，但不是唯一瓶颈；只值得做低成本 top-K、检索权重、few-shot 数量和示例顺序实验。
4. 第三阶段前移为重点备选：confusion-aware 或候选内判别 prompt。由于候选命中率明显高于最终准确率，当前更可能存在“正确 label 在候选中但模型选错”的损失，应优先分析混淆标签和 prompt 判别能力。
