...
3. 第二阶段调整为“候选召回与 prompt 参数小步实验”，而不是完整 prompt compiler。当前阶段 1.5 诊断显示候选集 gold 命中率 94.2%，top20 召回 96.5%，平均候选数 13.6，说明检索仍有改进空间，但不是唯一瓶颈；只值得做低成本 top-K、检索权重、few-shot 数量和示例顺序实验。
